...
file.prot <- file.path(dir.prot, "meso_proteome_ptm_all_log2_dqnorm.tsv.gz")
...
des <- subset(des, des$Group.3p21 == 0)
ids <- des$Sample.ID
...
### WRITE OUTPUT ---
file.output1 <- file.path(dir.output, "meso_correlation_ptm_BAP1INTACT_cna_mrna.tsv")
file.output2 <- file.path(dir.output, "meso_correlation_ptm_BAP1INTACT_cna_protein.tsv")
...
### RE-LOAD FILES ----
file.dat1 <- file.path(dir.output, "meso_correlation_ptm_BAP1INTACT_cna_mrna.tsv")
file.dat2 <- file.path(dir.output, "meso_correlation_ptm_BAP1INTACT_cna_protein.tsv")
...
### WRITE OUTPUT ---
file.output <- file.path(dir.output, "meso_correlation_ptm_BAP1INTACT_cna-mrna_cna-protein.tsv")
...
file.output <- file.path(dir.output, "meso_protein_ptm_BAP1INTACT_attenuated_genes.tsv")
write.table(d$Gene, file.output, sep="\t", row.names=FALSE, col.names=FALSE, quote=FALSE)

### PLOT ---
file.plot <- file.path(dir.plot, "meso_correlation_ptm_BAP1INTACT_cna-mrna_cna-protein.pdf")
