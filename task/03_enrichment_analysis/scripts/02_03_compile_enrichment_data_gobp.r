...
### SET PATHS ----------------------------------------------------------------------------------------------
#dir.wrk <- file.path("//jbrcsrv009/CollinsGroup/Raunak/projects/MESO_peritoneal/task/03_enrichment_analysis")
...
### DEFINE FILE --------------------------------------------------------------------------------------------
file.name <- "enrichment_c5.bp.v5.0.symbols.txt"
...
write.table(mat, file.path(dir.data, "MESO_OUTLIERS_p01_pathway_enrichment_gobp.tsv"), sep="\t", row.names=T, col.names=NA, quote=F)

### PLOT PATHWAYS HEATMAP ----------------------------------------------------------------------------------
jColFun <- colorRampPalette(brewer.pal(n = 9, "Greys"))

file.plot <- file.path(dir.plot, "MESO_OUTLIERS_p01_pathway_heatmap_GOBP.pdf")
