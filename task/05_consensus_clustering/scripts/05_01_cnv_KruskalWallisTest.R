...
write.table(df_kw_sig, file.path(dir.output, "meso_cnv_KruskalWallisTest_results.tsv"), sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
write.table(df_kw_sig$Feature, file.path(dir.output, "meso_cnv_KruskalWallisTest_features.txt"), sep="\t", row.names=FALSE, col.names=FALSE, quote=FALSE)

#> dim(df_kw_sig)
#[1] 772   4

### ADD ANNOTATIONS ----
file.annot <- file.path(dir.output, "meso_cnv_KruskalWallisTest_features.tsv")
...
df <- read.delim(file.path(dir.output, "meso_cnv_KruskalWallisTest_results.tsv"), header=T, stringsAsFactors=F)
...
write.table(df, file.path(dir.output, "meso_cnv_KruskalWallisTest_results_supplementarytable.tsv"), sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)

### SUBSET CNV DATA ---
dat <- subset(dat, rownames(dat) %in% df$Feature)
write.table(dat, file.path(dir.data, "meso_cnv_KruskalWallisTest_cnv.tsv"), sep="\t", row.names=TRUE, col.names=NA, quote=FALSE)


#### LOAD DATA ----
dat <- read.delim(file.path(dir.data, "meso_cnv_KruskalWallisTest_cnv.tsv"), header=T, stringsAsFactors=FALSE)
...
file.plot <- file.path(dir.plot, "heatmap_cnv_KruskalWallisTest_PeM.pdf")
pdf(file.plot, height=4, width=5)
	heatmap.2(as.matrix(dat), 
          ...
          cexCol=1, cexRow=1, symbreaks=TRUE, margin=c(10,10),
