#### DEFINE PATH ------------------------------------------------------
dir.wrk <- file.path("/collinsgroup/Raunak/projects/MESO_peritoneal/task/05_consensus_clustering")
dir.data <- file.path(dir.wrk, "data")
dir.plot <- file.path(dir.wrk, "plot")
dir.output <- file.path(dir.wrk, "output")
dir.expr <- file.path("/collinsgroup/Raunak/projects/MESO_peritoneal/data/expression/main_calls")
...
### SELECT FEATURES ----------------------------------------------------------
features <- c("3prime_overlapping_ncrna",
				"antisense","lincRNA","non_coding",
...
### Consensus Cluster ---------------------------------------------------
dir.batch <- file.path(dir.plot, "03_MESO_consensus_cluster_expression_lncs")
...
#### Since, optimum k = 2 -----------------------------------------------
dclass <- subset(dclass, dclass$k == 5)
dclass <- dclass[order(dclass$Class, decreasing=F),]
write.table(dclass, file.path(dir.output, "meso_consensus_cluster_expression_lncs_cluster.tsv"), sep="\t", row.names=F, col.names=T, quote=F)
...
### PLOT -----------------------------------------------
file.plot <- file.path(dir.plot, "meso_dendogram_expression_lncs.pdf")
